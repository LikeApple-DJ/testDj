...
| 调用人识别 | 前端用户名输入框 | 用户进入页面时输入姓名，后续调用自动携带 |
...
### 4.1 页面布局
- 顶部：用户名输入框（用户首次进入时输入姓名、部门、层级、人员类型，全局生效）
- 顶部下方：3个 Tab 切换（HelloWorld / 哈希算法 / 冒泡排序）
- 中部：对应 Tab 的执行结果展示区
- 右上角：导出按钮
- 底部：统计报表区域（折线图 + 饼图 + 柱状图）

### 4.2 调用人信息流转
- 用户在页面顶部输入框中填写姓名、部门、层级、人员类型
- 信息存储在 Vue 全局状态（Pinia store）中
- 每次调用 API 时，自动从 store 读取并附加到请求参数中
- 后端 `/api/track` 接口接收并持久化到数据库
